...
exports.getStoptimesByStop = (agency_key, route_id, stop_id, direction_id, cb) => {
    ...
    CalendarDate
      ...
      .exec(function(err, services) {
        ...
        if (!services || !services.length) {
          // todaysServices = service_ids;
          return cb();
        }

        holidayServices.removed = _.find(services, (o) => {return o.exception_type === 2;})
        holidayServices.added = _.find(services, (o) => {return o.exception_type === 1;})

        /*services.forEach((service) => {
          if(service.exception_type == 2 && service_ids.indexOf(service.service_id) !== -1 ){
            service_ids.splice(service_ids.indexOf(service.service_id), 1);
          } else {
            todaysServices.push(service.service_id);
          }
        });
        todaysServices = todaysServices.concat(service_ids);*/
        return cb();
      })
  }
  ...
  function findServices(cb) {
    ...
    if(agency_key === 'lirr') {
      console.log(`Calendar does not Exist for ${agency_key}`);
      return cb();
    }
    //build query
  ...
  function findTrips(cb) {
    ...
    if(agency_key === 'lirr') {
      if(todaysServices.added.length > 0){
        query['service_id']  = {
          '$in' : todaysServices.added,
          '$nin' : todaysServices.removed,
        }
      } else {
        query['service_id']  = {
          '$nin' : todaysServices.removed
    ...
    Trip
      .find(query)
      // .where('service_id').in(todaysServices)
      .exec((err, trips) => {
        if (err) return cb(err);
